...
- (void) sendEvent: (NSEvent*) theEvent  { 
	/* Hi-jack key presses and search for registered hot-key presses 
	 *	that we will use to send a hot-key press notification 
	 */
    if (([theEvent type]	== NSSystemDefined) && ([theEvent subtype] == kVtEventHotKeyPressedSubtype)) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName: kVtNotificationOnKeyPress 
																														object: [NSValue value: &hotKeyRef 
																																			withObjCType: @encode(EventHotKeyRef)]];		
    }
